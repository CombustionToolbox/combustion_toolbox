...
%     'H', 'OH', 'NO', 'NO2', 'N2O', 'He', 'Ar', 'C3H8', 'H2O2', 'HCN'};
%%
...
app.PD.TR.Value = 300;
app.PD.pR.Value = 1;
app.PD.phi.Value = 2:0.01:3.5;
% app.PD.phi.Value = 2:0.01:2.64;
...
switch Problem_selected
    ...
    case 'SP' % * SP: Isentropic (i.e., adiabatic) compression/expansion to a specified p
        ...
        app.PD.pP_vector.Value = 2:1:20; app.PD.phi.Value = 1*ones(1,length(app.PD.pP_vector.Value));
...
% app.PD.R_Fuel = 0; app.PD.phi_t = 1;
...
FO_st = sum(app.PD.R_Fuel(:,1))/(x+y/4-z/2);
%% PROBLEM TYPE
...
%         else
%             [app.PS.strR{i},app.PS.strP{i},app.TN.guess] = SolveProblemDET_fast(app.PS.strR{i},app.PD.phi.Value(i),app.PD.pR.Value,app.PD.TR.Value,app.TN.guess,app.PS.strR{i+1},app.PS.strP{i+1},app.E,app.S,app.C,app.M,app.PD,app.TN,strThProp);
%         end   
end
%% DISPLAY RESULTS
...
closing(app.PS.strP,app.PD.phi.Value,app.M.display_species,app.Misc.timer_0,app.S.NameSpecies,app.C.mintol_display);
%% EXCEL I/O
