...
function self = Index_fixed_Species(self)
    ...
    self.S.ind_CO2 = find_ind({'CO2'}, self.S.namespecies);
    self.S.ind_CO  = find_ind({'CO'}, self.S.namespecies);
    self.S.ind_H2O = find_ind({'H2O'}, self.S.namespecies);
    self.S.ind_H2  = find_ind({'H2'}, self.S.namespecies);
    self.S.ind_O2  = find_ind({'O2'}, self.S.namespecies);
    self.S.ind_N2  = find_ind({'N2'}, self.S.namespecies);
    self.S.ind_He  = find_ind({'He'}, self.S.namespecies);
    self.S.ind_Ar  = find_ind({'Ar'}, self.S.namespecies);
    self.S.ind_Cgr  = find_ind({'Cbgrb'}, self.S.namespecies);
...
function self = Stoich_Matrix(self)
    self.C.A0.value = zeros(self.S.Nspecies,self.E.NE);
    self.C.M0.value = zeros(self.S.Nspecies,12);
    for i=1:self.S.Nspecies
        txFormula = self.strThProp.(self.S.namespecies{i,1}).txFormula;
        self.strThProp.(self.S.namespecies{i,1}).Element_matrix = set_element_matrix(txFormula,self.E.elements);
        self.C.A0.value(i,self.strThProp.(self.S.namespecies{i,1}).Element_matrix(1,:)) = self.strThProp.(self.S.namespecies{i,1}).Element_matrix(2,:);
        self.C.M0.value(i,10) = self.strThProp.(self.S.namespecies{i,1}).swtCondensed;    
    end
end
    ...
    if self.Misc.FLAG_FIRST
        ...
        if self.M.Lminors > 0
            ...
            for n=self.M.Lminors:-1:1
                ...
                self.M.ind_minor(n) = find(strcmp(self.S.namespecies, self.M.minors_products{n}));
            end
