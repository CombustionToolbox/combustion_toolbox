classdef PlotComposition < combustiontoolbox.utils.display.Canvas
    % PlotComposition
    %
    % Subclass of Canvas for plotting molar fraction composition
    % against a selected variable (e.g., equivalence ratio).
    ...
    methods
        ...
        function obj = PlotComposition(config)
            % Constructor

            if nargin < 1
                % Default values
                defaultInnerPosition = [0.15, 0.15, 0.65, 0.75];
                defaultOuterPosition = [0.15, 0.15, 0.65, 0.75];
        ...
        function [ax, fig] = plot(obj, mixture, x_var, x_field, y_field, varargin)
            ...
            [species, listSpecies] = obj.get_displaySpecies(mixture, displaySpecies);
            ...
            [species, index_species_CT] = obj.clean_displaySpecies(mix1.Xi, listSpecies, index_species_CT, mintolDisplay, obj.FLAG_PLOT_VALIDATION);
            ...
            colorbw = combustiontoolbox.utils.extensions.brewermap(numColors, config.colorpalette);
            ...
            for i = 1:length(species)
                ...
                if FLAG_Y_AXIS
                    plot(ax, mix1.(x_field), mix1.(y_field)(index_species_CT(i), :), 'LineWidth', config.linewidth, 'color', colorbw(k, :), 'LineStyle', config.lineStyles{z});
                else
                    plot(ax, mix1.(x_field)(index_species_CT(i), :), mix1.(y_field), 'LineWidth', config.linewidth, 'color', colorbw(k, :), 'LineStyle', config.lineStyles{z});
                end
            ...
            if obj.FLAG_PLOT_VALIDATION
                ...
                for i = 1:length(species)
                    ...
                    if FLAG_Y_AXIS
                        plot(ax, mix2.(x_field)(1:nfrec:end), mix2.(y_field)(i, 1:nfrec:end), config.symbolStyles{z}, 'LineWidth', config.linewidth, 'color', colorbw(k, :), 'MarkerFaceColor', 'white');
                    else
                        plot(ax, mix2.(x_field)(i, 1:nfrec:end), mix2.(y_field)(1:nfrec:end), config.symbolStyles{z}, 'LineWidth', config.linewidth, 'color', colorbw(k, :), 'MarkerFaceColor', 'white');
                    end
                ...
                for i = 1:length(species)
                    h(i) = plot(ax, NaN, NaN, [config.lineStyles{z}, config.symbolStyles{z}], 'LineWidth', config.linewidth, 'color', colorbw(k, :), 'MarkerFaceColor', 'auto');
                    k = k + 1;
            ...
            if isempty(config.title)
                config.title = getTitle(mixture);
            else
                config.title = sprintf('%s - %s', config.title, getTitle(mixture));
            end
    ...
    methods (Access = private, Static)
        ...
        function [species, listSpecies] = get_displaySpecies(mixture, displaySpecies)
