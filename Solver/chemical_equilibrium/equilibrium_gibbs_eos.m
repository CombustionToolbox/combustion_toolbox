...
%         itMax = 50 + round(S.NS/2);
        ...
        while STOP > TN.tolN && it < itMax
            it = it + 1;
            % Estimation of the molar volume (v_molar)
            [v_molar, a_mix, b_mix] = eos_PR(self, S.LS, N0(temp_ind_nswt, 1) / sum(N0(:, 1)), TP, pP);
            % Non-ideal contribution of chemical potential
            muRT_ex = mu_ex_eos(self, TP, pP, v_molar, a_mix, b_mix) / R0TP;
            % Chemical potential
