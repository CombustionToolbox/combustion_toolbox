...
%     Aratio = 1.1:0.1:2.6;
    ...
    for i = length(Aratio):-1:1
        ...
        results_CT = run_CT('ProblemType', 'ROCKET', 'TR', 298.15, 'pR', 22,...
                            ...
                            'S_Inert', [], 'EquivalenceRatio', 1:0.1:4, 'tolN', tolN,...
...
%         results_CEA.OF = OF(:, i);
        ...
        if FLAG_SAVE
            ...
            stack_trace = dbstack;
            filename = strcat('RP1_LOX_molar_OF_', sprintf('%d', i));
            saveas(fig1, strcat(folderpath, 'run_validation_ROCKET_CEA_OF_16'), 'svg');
        end
    end
