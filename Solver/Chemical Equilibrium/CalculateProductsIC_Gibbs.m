function [P_IC,DeltaNP, phi_c] = CalculateProductsIC_Gibbs(P_CC,TP,pP,vP,phi,minor_products,phi_c,FLAG_SOOT,M0,A0,E,S,C,M,PD,TN,strThProp)
...
%             if NCgr <= 0 || ~t
            if NCgr < 1e-8
                if t 
                    NCgr = 0;
                    [NCO2_0,NCO_0,NH2O_0,NH2_0,NO2_0,NN2_0,~,~,NCgr_0,~,FLAG_SOOT] = CalculateProductsCC(NatomE,phi,0,TP,pP,E.Elements,TN.factor_c,PD.Fuel,strThProp);
                    N_minor_C_0 = 0;
                    N_minor_H_0 = 0;
                    N_minor_O_0 = 0;
                    N_minor_N_0 = 0;
                    ...
                    N_minor_O_0_nswt = 0;
                    N_minor_N_0_nswt = 0;
                    if any([NCO2_0, NH2O_0, NH2_0, NO2_0, NN2_0, NCgr_0] < 0)
                        if it>1
                            NCO_0  = NCO_old;
                            NCO2_0 = NCO2_old;
                            NH2O_0 = NH2O_old;
                        ...
                        P_CC = SetSpecies(C.M0.Value,S.List_Compute_Species,[NCO2_0,NCO_0,NH2O_0,NH2_0,NO2_0,NN2_0,NHe,NAr,NCgr_0]',TP,S.idx_fixed,strThProp);
                        NP = sum(P_CC(:,1).*(1-P_CC(:,10))); % Sum of num of moles of gases-(1-swt), with swt == condensed phase.
%                         x = NatomE(E.ind_C); %  6 = find(strcmp(Elements,'C'))
%                         y = NatomE(E.ind_H); %  1 = find(strcmp(Elements,'H'))
%                         z = NatomE(E.ind_O); %  8 = find(strcmp(Elements,'O'))
...
%                 NH2  = NH2_old +relax*(NH2 -NH2_old);
%                 if abs((NCO2-NCO2_old)/(NCO2)) > 1e-1
%                     NCO2 = NCO2_old+0.0001*relax*(NCO2-NCO2_old);
%                     NH2O = NH2O_old+0.0001*relax*(NH2O-NH2O_old);
%                 end
...
%                 NH2  = (NH2_old-NH2)/log(NH2_old/NH2);
            ...
            if NCgr < 0 || ~isreal(NCgr), NCgr = 0.75*NCgr_old ; end
