function [NCO2P_0,NCOP_0,NH2OP_0,NH2P_0,NO2P_0,NN2P_0,NHeP_0,NArP_0,NCgrP_0,phi_c,FLAG_SOOT] =  CalculateProductsCC_eps(NatomE,phi,TP,Elements,factor_c,Fuel,strThProp)
...
%     phi_c = -(((-1+eps)*(4*Fuel.x+Fuel.y))/(2*(1+eps)*Fuel.x+eps^2*Fuel.y)); % C_x H_y
    end
else
    phi_c = 1.1*phi;
end
...
%     else
        ...
        Fuel.eps = NCO2P_0/NCOP_0;
        phi_c2 = -(((-1+Fuel.eps)*(4*Fuel.x+Fuel.y-2*Fuel.z))/(2*(1+Fuel.eps)*Fuel.x+Fuel.eps^2*Fuel.y-2*Fuel.z+2*Fuel.eps*Fuel.z)); % C_x H_y O_z
        if phi_c2 < phi
            phi_c = phi_c2;
