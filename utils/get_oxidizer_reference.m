function self = get_oxidizer_reference(self)
    % Get oxidizer of reference for computations with the equivalence ratio
    %
    % Args:
    %     self (struc): Data of the mixture, conditions, and databases
    %
    % Returns:
    %     self (struc): Data of the mixture, conditions, and databases, 
    ...
    if ismember(self.PD.S_Oxidizer, 'O2')
        self.S.ind_ox_ref = find_ind(self.S.LS, 'O2');
        self.S.atoms_ox_ref = 2;
    elseif ismember(self.PD.S_Oxidizer, 'O2bLb')
