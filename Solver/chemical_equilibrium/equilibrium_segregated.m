function [N0, STOP] = equilibrium_segregated(self, pP, TP, mix1, N0)
% function [N0,STOP] = equilibrium_segregated(app, N0, phi, pP, TP, vP, phi_c, FLAG_SOOT)
...
TN = self.TN;
DB = self.DB;
ind_minors = ~ismember(self.S.LS, self.S.LS_fixed);
LS = S.LS;
phi = mix1.phi;
phi_c = mix1.phi_c;
% -----------------------------------
...
% Get index of major species
ind_CO2 = find_ind(LS, 'CO2');
ind_CO = find_ind(LS, 'CO');
ind_H2O = find_ind(LS, 'H2O');
ind_H2 = find_ind(LS, 'H2');
ind_O2 = find_ind(LS, 'O2');
ind_N2 = find_ind(LS, 'N2');
ind_Cgr = find_ind(LS, 'Cbgrb');


M.major_CH4 = true;
M.major_OH = true;
M.ind_m_CH3 = find_ind(LS, 'CH3');
M.ind_m_CH4 = find_ind(LS, 'CH4');
M.ind_m_H = find_ind(LS, 'H');
ind_minors(ind_Cgr) = false; % Soot is a fixed species
M.ind_minors = find(ind_minors);
minors_products = self.S.LS(ind_minors);
M.Lminors = length(minors_products);
g0_minors = set_g0(minors_products, TP, DB);
C = setup_segregated_solver(self, minors_products);
...
NP = sum(N0(:, 1) .* (1-N0(:, 2))); % Sum of num of moles of gases-(1-swt), with swt == condensed phase.
% NP = sum(N0(:, 1));
...
M0 = zeros(length(LS), 2);
M0(:, 2) = N0(:, 2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CALCULATION OF GIBBS FREE ENERGY, CONSTANTS EQUILIBRIUM AND OTHERS
g_CO2 = species_g0('CO2',TP,DB) * 1e3;
g_CO  = species_g0('CO',TP,DB) * 1e3;
g_H2O = species_g0('H2O',TP,DB) * 1e3;
DG0_I    = (g_CO-g_CO2);
DG0_II   = -g_H2O;
k1  = exp(-DG0_I/R0TP);
k2  = exp(-DG0_II/R0TP);
if M.major_CH4
    g_CH4 = species_g0('CH4',TP,DB) * 1e3;
    DG0_VIII   = (species_g0('CH3',TP,DB)-species_g0('H',TP,DB)-g_CH4) * 1e3;
    k8  = exp(-DG0_VIII/R0TP);
end
if M.major_OH
    g_OH = species_g0('OH',TP,DB) * 1e3;
    DG0_XI   = g_OH;
...
if phi<=1 && M.Lminors>0 % case of lean-to-stoichiometric mixtures
    ...
    for n = M.Lminors:-1:1
        DG0_III(n) = (species_g0(minors_products{n},TP,DB)-C.alpha(n)*g_CO2 ...
            -(C.beta(n)/2)*g_H2O);
    end
    k3 = exp(-DG0_III/R0TP);
elseif phi>1  % case rich mixtures
    ...
    elseif phi < phi_c*TN.factor_c && ~FLAG_SOOT% general case of rich mixtures with hydrogens (H) and carbons (C)
        ...
        DG0_IV = (g_CO+g_H2O-g_CO2);
        k4 = exp(-DG0_IV/R0TP);
    elseif phi >= phi_c*TN.factor_c || FLAG_SOOT
        DG0_V = (g0_minors ...
            -(C.gamma - C.alpha - C.beta/2) * g_CO2 ...
            -(C.beta/2) * g_H2O ...
            -(2*C.alpha - C.gamma + C.beta/2) * g_CO);
        if M.Lminors > 0
            k5 = exp(-DG0_V/R0TP);
            DNfactor_V = 1-C.alpha-C.beta/2-C.omega/2;
        end
...
%%% CHEMICAL EQUILIBRIUM
    function [N0,STOP] = incomplete_phi_1(N0,E,S,M,C)
        STOP = 1;
        while abs(STOP/NP) > C.tolN && it<itMax
            it = it+1;
            ...
            if M.Lminors>0
                ...
                %             for n =M.Lminors:-1:1
                %                 Ni_old = N0_old(M.ind_minors(n),1);
                %                 if Ni_old ~=0
                %                     Ni(n) = Ni_old+relax*(Ni(n)-Ni_old);
                %                 end
                %                 N0(M.ind_minors(n),[DB.(minors_products{n}).Element_matrix(1,:),1]) = [Ni(n)*DB.(minors_products{n}).Element_matrix(2,:),Ni(n)];
                %             end
%                 if M.major_OH && STOP
%                     Ni(M.ind_m_OH) = sqrt(NH2*NO2*k11*zeta^(-3/2));
%                 end
                Ni(Ni>NP_old) = 0.75*Ni(Ni>NP_old);
                Ni_old = N0_old(M.ind_minors,1)';
                aux = find(Ni_old~=0);
                if ~isempty(aux)
                    Ni(aux) = Ni_old(aux)+relax*(Ni(aux)-Ni_old(aux));
                end
                N0(M.ind_minors,1) = Ni;
                % disp(sortrows([cell2table(S.NameSpecies(N0(:, 1)>0.000001), 'VariableNames', {'Species'}), array2table(N0(N0(:, 1)>0.000001), 'VariableNames', {'N'})], 2, 'descend'))
            end
        % disp([sum(N0(:, 1).*A0(:,E.ind_C)), sum(N0(:, 1).*A0(:,E.ind_H)), sum(N0(:, 1).*A0(:,E.ind_O)), sum(N0(:, 1).*A0(:,E.ind_N))])
        % Correction of the number of moles of CO2, H2O, O2 and N2 from atom
        % conservation equations
        NCO2_old = NCO2;
        NCO2 = NCO2_0-sum(N0(:, 1).*A0(:,E.ind_C)); % C-atom conservation
        if TP > 3000, NCO2 = NCO2_old+relax*(NCO2-NCO2_old); end
        if NCO2 < 0, NCO2 = 0.75*NCO2_old; end
        
        NH2O_old = NH2O;
        NH2O = NH2O_0-sum(N0(:, 1).*A0(:,E.ind_H))/2; % H-atom conservation
        ...
        NO2_old = NO2;
        NO2  = NO2_0+NCO2_0+NCO_0/2+NH2O_0/2-sum(N0(:, 1).*A0(:,E.ind_O))/2; % O-atom conservation
        if TP > 3000, NO2 = NO2_old+relax*(NO2-NO2_old); end
        if NO2 < 0, NO2 = 0.75*NO2_old; end
        
        N0(ind_O2,1) = NO2;
        NN2_old = NN2;
        NN2  = NN2_0 -sum(N0(:, 1).*A0(:,E.ind_N))/2; % N-atom conservation
        ...
        NP = sum(N0(:, 1).*(1-N0(:, 2)));
%         NP = sum(N0(:, 1));
        STOP = norm([NP-NP_old, x-sum(N0(:, 1).*A0(:,E.ind_C)), y-sum(N0(:, 1).*A0(:,E.ind_H)), z-sum(N0(:, 1).*A0(:,E.ind_O)), w-sum(N0(:, 1).*A0(:,E.ind_N))]);
        % relax = abs(relax*(1-STOP/NP));
        % disp(sortrows([cell2table(S.NameSpecies(N0(:, 1)>0.000001), 'VariableNames', {'Species'}), array2table(N0(N0(:, 1)>0.000001), 'VariableNames', {'N'})], 2, 'descend'))
        end
    end % PHI <= 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [N0,STOP] = incomplete_phi_2(N0,E,S,M,C)
        STOP = 1;
        while abs(STOP/NP) > C.tolN && it<itMax 
            it = it+1;
            % Initialization of the product matrix for incomplete combustion (IC)
...
%                 end
            ...
            NH2O = NH2O_0-2*(NO2+sum(N0(:, 1).*A0(:,E.ind_O))/2);
            NH2  = NH2_0 +2*(NO2+sum(N0(:, 1).*A0(:,E.ind_O))/2-sum(N0(:, 1).*A0(:,E.ind_H))/4);
            NN2  = NN2_0 -sum(N0(:, 1).*A0(:,E.ind_N))/2; % N-atom conservation
...
%             NP = sum(N0(:, 1));
%             STOP = NP-NP_old;
            STOP = norm([NP-NP_old, y-sum(N0(:, 1).*A0(:,E.ind_H)), z-sum(N0(:, 1).*A0(:,E.ind_O)), w-sum(N0(:, 1).*A0(:,E.ind_N))]);
            % relax = abs(relax*(1-STOP/NP));
        end
    end % 1 < PHI < PHI_C AND ~H
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [N0,STOP] = incomplete_phi_3(N0,E,S,M,C)
        STOP = 1;
        while abs(STOP/NP) > C.tolN && it<itMax 
            it = it+1;
            % Initialization of the product matrix for incomplete combustion (IC)
...
%                        +log(zeta).*DNfactor_V);
            ...
            NCO2 = NCO2_0-2*(NO2-3*sum(N0(:, 1).*A0(:,E.ind_C))/2+sum(N0(:, 1).*A0(:,E.ind_O))/2);
            NCO  = NCO_0 +2*(NO2-sum(N0(:, 1).*A0(:,E.ind_C))+sum(N0(:, 1).*A0(:,E.ind_O))/2);
            NN2  = NN2_0 -sum(N0(:, 1).*A0(:,E.ind_N))/2; % N-atom conservation
...
%             NP = sum(N0(:, 1));
%             STOP = NP-NP_old;
            STOP = norm([NP-NP_old, x-sum(N0(:, 1).*A0(:,E.ind_C)), z-sum(N0(:, 1).*A0(:,E.ind_O)), w-sum(N0(:, 1).*A0(:,E.ind_N))]);
            % relax = abs(relax*(1-STOP/NP));
        end
    end % 1 < PHI < PHI_C AND ~C
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [N0,STOP] = incomplete_phi_4(N0,E,S,M,C)
                STOP = 1;
        while abs(STOP/NP) > C.tolN && it<itMax 
            it = it+1;
            % Initialization of the product matrix for incomplete combustion (IC)
...
%             sum(N0(:, 1).*A0(:,E.ind_N))
            ...
            a = NCO2_0+NCO_0-sum(N0(:, 1).*A0(:,E.ind_C));
            b = NH2O_0+NH2_0-sum(N0(:, 1).*A0(:,E.ind_H))/2;
            c = NCO_0+NH2_0+2*(NO2-sum(N0(:, 1).*A0(:,E.ind_C))-sum(N0(:, 1).*A0(:,E.ind_H))/4+sum(N0(:, 1).*A0(:,E.ind_O))/2);
            d = b-c;
...
%             NP = sum(N0(:, 1));
            STOP = norm([NP-NP_old, x-sum(N0(:, 1).*A0(:,E.ind_C)), y-sum(N0(:, 1).*A0(:,E.ind_H)), z-sum(N0(:, 1).*A0(:,E.ind_O)), w-sum(N0(:, 1).*A0(:,E.ind_N))]);
%             if phi> 1.1, % relax = abs(relax*(1-STOP/NP)); end
        end 
%         figure(1)
%         hold on;
%         T = linspace(300,6000);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [N0, STOP] = incomplete_phi_5(N0,E,S,M,C)
        error('NOT IMPLEMENTED YET');
%         STOP = 1;
%         while abs(STOP/NP) > C.tolN
%             % Initialization of the product matrix for incomplete combustion (IC)
%             NP_old   = NP;
%             N0_old = N0;
%             
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [N0,STOP] = incomplete_phi_6(N0,E,S,M,C)
        STOP = 1;
        while abs(STOP/NP) > TN.tolN && it<itMax 
            it = it+1;
            % Initialization of the product matrix for incomplete combustion (IC)
            NP_old   = NP;
            N0_old = N0;
...
%             NO2 = 0;
            ...
            a = NCO2_0+NCO_0+NCgr_0-sum(N0(:, 1).*A0(:,E.ind_C));
            b = NH2O_0+NH2_0-sum(N0(:, 1).*A0(:,E.ind_H))/2;
            c = 2*NCO2_0+NCO_0+NH2O_0-2*NO2-sum(N0(:, 1).*A0(:,E.ind_O));
...
%                     NH2_0 = b-NH2O;
                    ...
                    if it>1
                        ...
                        N_minor_C_0 = sum(N0_old(:,1).*A0(:,E.ind_C))-NCO2_0-NCO_0-NCgr_0;
                        N_minor_H_0 = sum(N0_old(:,1).*A0(:,E.ind_H))-2*NH2O_0-2*NH2_0;
                        N_minor_O_0 = sum(N0_old(:,1).*A0(:,E.ind_O))-2*NO2_0-2*NCO2_0-NCO_0-NH2O_0;
                        N_minor_N_0 = sum(N0_old(:,1).*A0(:,E.ind_N))-2*NN2_0;
                        
                        N_minor_C_0_nswt = sum(N0_old(:,1).*A0(:,E.ind_C).*(1-N0(:, 2)))-NCO2_0-NCO_0-NCgr_0;
                        N_minor_H_0_nswt = sum(N0_old(:,1).*A0(:,E.ind_H).*(1-N0(:, 2)))-2*NH2O_0-2*NH2_0;
                        N_minor_O_0_nswt = sum(N0_old(:,1).*A0(:,E.ind_O).*(1-N0(:, 2)))-2*NO2_0-2*NCO2_0-NCO_0-NH2O_0;
                        N_minor_N_0_nswt = sum(N0_old(:,1).*A0(:,E.ind_N).*(1-N0(:, 2)))-2*NN2_0;
                    else
%                         NCO_0  = NCO;
%                         NCO2_0 = NCO2;
%                         NH2O_0 = NH2O;
...
%                         N_minor_C_0 = sum(N0(:, 1).*A0(:,E.ind_C));
%                         N_minor_H_0 = sum(N0(:, 1).*A0(:,E.ind_H));
%                         N_minor_O_0 = sum(N0(:, 1).*A0(:,E.ind_O));
%                         N_minor_N_0 = sum(N0(:, 1).*A0(:,E.ind_N));
%                         
%                         N_minor_C_0_nswt = sum(N0(:, 1).*A0(:,E.ind_C).*(1-N0(:, 2)));
%                         N_minor_H_0_nswt = sum(N0(:, 1).*A0(:,E.ind_H).*(1-N0(:, 2)));
%                         N_minor_O_0_nswt = sum(N0(:, 1).*A0(:,E.ind_O).*(1-N0(:, 2)));
%                         N_minor_N_0_nswt = sum(N0(:, 1).*A0(:,E.ind_N).*(1-N0(:, 2)));
...
%                     NO2 = exp(log(NO2_old) +relax*(log(NO2) -log(NO2_old)));
                ...
                a = NCO2_0+NCO_0+NCgr_0+N_minor_C_0-sum(N0(:, 1).*A0(:,E.ind_C));
                b = NH2O_0+NH2_0+N_minor_H_0/2-sum(N0(:, 1).*A0(:,E.ind_H))/2;
                c = NCO_0+NH2_0+2*(NCgr_0+N_minor_C_0+N_minor_H_0/4-N_minor_O_0/2)+2*(NO2-NO2_0-sum(N0(:, 1).*A0(:,E.ind_C))-sum(N0(:, 1).*A0(:,E.ind_H))/4+sum(N0(:, 1).*A0(:,E.ind_O))/2);
                ...
                NN2  = NN2_0+N_minor_N_0/2-sum(N0(:, 1).*A0(:,E.ind_N))/2; % N-atom conservation
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [N0,STOP] = incomplete_phi_7(N0,E,S,M,C)
        STOP = 1;
        while abs(STOP/NP) > C.tolN && it<itMax 
            it = it+1;
            % Initialization of the product matrix for incomplete combustion (IC)
            ...
            a = NCO2_0+NCO_0+NCgr_0-sum(N0(:, 1).*A0(:,E.ind_C));
            b = NH2O_0+NH2_0-sum(N0(:, 1).*A0(:,E.ind_H))/2;
            c = 2*NCO2_0+NCO_0+NH2O_0+2*NO2_0-2*NO2-sum(N0(:, 1).*A0(:,E.ind_O));
