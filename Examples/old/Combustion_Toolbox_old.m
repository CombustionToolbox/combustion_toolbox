...
self.PD.S_Fuel = {'CH4'};
%  self.PD.N_Fuel = 1;
% * DEFINE OXIDIZER | default: self.PD.N_Oxidizer -> phi_t/phi
self.PD.S_Oxidizer = {'O2'};
% self.PD.N_Oxidizer = 2;
% * DEFINE DILUENTS/INERTS | default: self.PD.N_Inert -> phi_t/phi .* proportion_inerts_O2
self.PD.S_Inert = {'N2', 'Ar', 'CO2'};
self.PD.proportion_inerts_O2 = [78.084, 0.9365, 0.0319] ./ 20.9476;
% self.PD.N_Inert = [7.4286, 0.0886, 0.0038];
%% PROBLEM TYPE
% * TP: Equilibrium composition at defined T and p
self = set_prop(self, 'pP', self.PD.pR.value, 'TP', 3000); 
% * HP: Adiabatic T and composition at constant p
self = set_prop(self, 'pP', self.PD.pR.value); 
% * SP: Isentropic (i.e., adiabatic) compression/expansion to a specified p
pP = 10:1:50; self = set_prop(self, 'pP', pP, 'phi', self.PD.phi.value(1) * ones(1, length(pP))); 
% pP = 10:1:50; self = set_prop(self, 'pP', 10*ones(1, length(self.PD.phi.value)));
% * TV: Equilibrium composition at defined T and constant v
self = set_prop(self, 'TP', 2000);
% * EV: Equilibrium composition at Adiabatic T and constant v
self = set_prop(self, 'pP', self.PD.pR.value);
% * SV: Isentropic (i.e., fast adiabatic) compression/expansion to a specified v
vP_vR = 0.5:0.01:2; self = set_prop(self, 'vP_vR', vP_vR, 'phi', self.PD.phi.value(1) * ones(1, length(vP_vR)));
% * SHOCK_I: CALCULATE PLANAR INCIDENT SHOCK WAVE
...
u1 = linspace(400, 6000, 1000); self = set_prop(self, 'u1', u1, 'phi', self.PD.phi.value(1) * ones(1, length(u1)));
% * DET: CALCULATE CHAPMAN-JOUGET STATE (CJ UPPER STATE)
% self.PD.TR_vector.value = self.PD.TR.value;
% * DET_OVERDRIVEN: CALCULATE OVERDRIVEN DETONATION
overdriven = 1:0.1:10;
