function [N0, STOP] = Equilibrium_soot(app, pP, TP, strR)
...
[A1, temp_NS0] = update_matrix_A1(A0, [], temp_NS, temp_NS0, temp_ind, temp_NG, temp_ind_E);
A22 = zeros(temp_NE + 1);
A0_T = A0';
...
while STOP > C.tolN && it < itMax
    ...
    % Apply correction
    N0(temp_ind_nswt, 1) = log(N0(temp_ind_nswt, 1)) + e * x(1:temp_NG);
    N0(temp_ind_swt, 1) = N0(temp_ind_swt, 1) + e * x(temp_NG+1:temp_NS);
    ...
    % Update matrix A1
    [A1, temp_NS0] = update_matrix_A1(A0, A1, temp_NS, temp_NS0, temp_ind, temp_NG, temp_ind_E);
...
function [A1, temp_NS0] = update_matrix_A1(A0, A1, temp_NS, temp_NS0, temp_ind, temp_NG, temp_ind_E)
...
function [N0, NP] = apply_antilog(N0, NP_log, temp_ind_nswt)
    N0(temp_ind_nswt, 1) = exp(N0(temp_ind_nswt));
    NP = exp(NP_log);
end
